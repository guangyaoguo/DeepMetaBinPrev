            ...
            with open(os.path.join(tdir, 'markers.hmmout.out'), 'w') as hmm_out_log:
                subprocess.check_call(
                    ['hmmsearch',
                     ...
                     '--cut_tc',
                     '--cpu', str(num_process),
                     os.path.split(__file__)[0] + '/utils/marker.hmm',
                    #  './utils/marker.hmm',
                     contig_output,
                     ],
                    stdout=hmm_out_log,
...
def read_bins(file):
    ...
    with open(file, 'r') as f:
        for l in f.readlines():
            items = l.split()
            bins[int(items[1])].append(items[0])
    return dict(bins)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--contig_path', type=str, default='/datahome/datasets/ericteam/csmxrao/DeepMetaBin/tmp/hlj10x/gmm_bins', help='Fasta path (.fasta)')
    ...
    for bin_path in fasta_bin:
        ...
        if cluster_num in issue_bins:
            ...
            if n_clusters <= 1:
                n_clusters = 2
            labels = bh_kmeans(bin_data, n_clusters, ml=ml_indices, cl=cl_indices, p=3, random_state=2021, time_limit=1800)
