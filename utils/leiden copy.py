...
def hnsw_index(features,num_threads, ef: int = 100, M: int = 16,
    ...
    label_id = np.arrange(num_elements)
...
def leiden_clustering_alg(namelist, ann_neighbor_indices, ann_distances, length_weight, max_edges, latent, bandwidth=0.1, lmode='l2', initial_list=None, is_membership_fixed=None, resolution_parameter=1.0, partgraph_ratio=50):
    ...
    res = RBERVertexPartition(g, weights=wei, initial_membership=initial_list, resolution_parameter=resolution_parameter, node_sizes=length_weight)
...
def get_length_weight(contignames):
    length_list = contignames.str.extract(r'length_(\d+)_').astype(int)
    return length_list

def cluster(logger, latent, contignames, threads, max_edges, prefix=None):
 
    num_workers = threads

    p = hnsw_index(logger, latent, num_workers, ef=max_edges * 10)

    ann_neighbor_indices, ann_distances = p.knn_query(embd, max_edges + 1, num_threads=num_workers)
    ...
    return leiden_clustering_alg(contignames, ann_neighbor_indices, ann_distances, length_weight, max_edges, norm_embeddings,
                                                                        bandwidth = 0.05, lmode = 'l2', partgraph_ratio = 50)/,``j 
